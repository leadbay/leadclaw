...
👉 **[Download the latest LeadMCP for Claude (.dxt)](https://github.com/leadbay/leadbay-mcp/releases/latest)**
...
|--------|------------------|-----------|-------|
| **`.dxt` / `.mcpb` bundle** | Download from [Releases](https://github.com/leadbay/leadbay-mcp/releases/latest), double-click → **Install** | Claude Desktop | One-click. The recommended path for end users. |
...
| **Claude Code plugin marketplace** | `/plugin marketplace add leadbay/leadbay-mcp` then `/plugin install leadbay@leadbay-mcp` | Claude Code | Registers the MCP server **and** installs auto-triggering skills. |

...
describe("checkForUpdate — 200 OK newer release", () => {
  it("parses tag_name, prefers the .dxt asset, persists state, populates cache", async () => {
    ...
    const fetchImpl = fakeFetch([
      {
        ...
        body: {
          ...
          html_url: "https://github.com/leadbay/leadbay-mcp/releases/tag/mcp-v0.10.2",
          assets: [
            { name: "leadbay-mcp-0.10.2.dxt", browser_download_url: "https://gh.example/0.10.2.dxt" },
            { name: "leadbay-mcp-0.10.2.mcpb", browser_download_url: "https://gh.example/0.10.2.mcpb" },
          ],
        },
      },
    ]);
    ...
    expect(info).toEqual({
      ...
      release_url: "https://github.com/leadbay/leadbay-mcp/releases/tag/mcp-v0.10.2",
    });
    const s = await store.read();

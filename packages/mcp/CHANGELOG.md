...
- **The bootstrap browser now opens under Claude Desktop's sanitized environment**: the flow launched the browser via `spawn("open" | "xdg-open" | "cmd", …)` — bare command names resolved through `PATH` — but Claude Desktop spawns extensions with a sanitized `PATH` that doesn't contain those launchers, so the spawn failed with `ENOENT` and no browser opened. `openInBrowser` now launches via the OS launcher's **absolute path** (`/usr/bin/open`, `%SystemRoot%\System32\cmd.exe`, `/usr/bin/xdg-open`), keeping the bare-command `PATH` lookup only as a fallback. If no launcher can be found, the first tool call returns an `AUTH_MISSING` envelope telling the user to restart the extension to retry sign-in.
